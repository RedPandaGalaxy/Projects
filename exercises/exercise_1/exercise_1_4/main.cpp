...
void makeDisc(int sides, std::vector<float>& vertices, std::vector<float>& color);
...
void setupShape(const unsigned int shaderProgram,unsigned int &VAO, unsigned int &vertexCount){
    ...
    createArrayBuffer(v, posVBO);

    
    createArrayBuffer( c, colorVBO);
...
void makeDisc(int sides, std::vector<float> &vertices, std::vector<float>& color    ) {
    ...
    for (int i = 0; i < sides; i++) {
        ...
        float colorA = 1 - i*(1.0/sides); 
        float colorB = 1 - ((i+1) * (1.0 / sides));
        std::cout << "CA: " << colorA << " CB: " << colorB << std::endl;
        /*
        if (i % 4)
        {
            colorA = colorA + colorB;
            colorB = colorA - colorB; //colorA
        ...
        */
        ...
        //          R                       G                           B
        color.push_back(1);       color.push_back(1);       color.push_back(1);

        //point A
        //          x                       y                          z
        vertices.push_back(x); vertices.push_back(y); vertices.push_back(0.0f);
        //          R                       G                           B
        color.push_back(colorA);       color.push_back(colorB);       color.push_back(1);
        //point B
        angle = alpha * ( i + 1 );
        x = cos(angle) * radius;
        y = sin(angle) * radius;
